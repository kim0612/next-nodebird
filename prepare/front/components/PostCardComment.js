...
const PostCardComment = ({post}) => {
  ...
  const submitNewComment = useCallback(()=>{
    console.log(`게시물 id: ${post.id} / 새 댓글: ${newComment} / ME nickname : ${me.nickname}`);
    dispatch(addCommentRequestAction({postId:post.id, content:newComment, meNickname:me.nickname}));
  }, [post, newComment, me]);
        ...
        <>
          ...
          >
            <Input.TextArea
              ...
              rows="4"
            />
            <Button type="primary" htmlType="submit" loading={addCommentLoading}>완료</Button>
          </Form>
